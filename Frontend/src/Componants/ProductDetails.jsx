...
//   // Animated badge – appears for "Trending" or specials:
//   const AnimatedBadge = ({ children }) => (
//     <motion.div
//       className="absolute top-2 right-2 px-3 py-1 rounded-full font-semibold text-xs tracking-wide text-yellow-900 bg-yellow-300 bg-opacity-90 shadow-lg z-20"
...
//                  bg-gradient-to-r from-blue-600 to-indigo-500 text-white outline-none border-2 border-white
...
//       whileHover={{ scale: 1.07, boxShadow: "0 12px 56px #6366f1" }}
//     >
...
//   return (
//     <div className="pt-20 max-w-7xl mx-auto px-3 py-6 min-h-screen relative bg-gradient-to-br from-indigo-100 via-white to-yellow-50">
//       {/* Animated Loading & Error */}
//       <AnimatePresence>
//         {loading && (
//           <motion.p
//             className="text-center p-4 text-xl font-medium text-blue-700"
...
//               <motion.div
//                 className="relative rounded-2xl w-full max-w-xs sm:max-w-md md:max-w-lg md:self-center shadow-xl mb-4 md:mb-0"
//                 initial={{ scale: 0.94, opacity: 0.9 }}
//                 animate={{ scale: 1, opacity: 1 }}
//                 whileHover={{ scale: 1.02 }}
//                 transition={{ duration: 0.45 }}
//                 style={{ zIndex: 1, background: "rgba(255,255,255,0.64)" }}
//               >
//                 {/* Animated Aura Glow */}
//                 <motion.div
...
//                   style={{
//                     background:
//                       "radial-gradient(circle at 60% 48%, #60a5fa55 10%, #a7f3d0bb 70%, #fff0 )",
//                     filter: "blur(24px)",
//                   }}
//                   animate={{
//                     opacity: [0.72, 1, 0.78],
...
//                     src={mainImage}
//                     alt={product.title}
//                     className="relative z-10 w-full aspect-square object-cover rounded-2xl shadow-2xl ring-2 ring-blue-100"
...
//                           : "border-transparent"
//                       }`}
//                     whileHover={{ scale: 1.18, boxShadow: "0 0 16px #bfdbfe" }}
//                     whileTap={{ scale: 0.96 }}
...
//                       filter:
//                         img === mainImage
//                           ? "drop-shadow(0px 0px 8px #38bdf8)"
//                           : undefined,
//                     }}
//                   />
//                 ))}
...
//                 rounded-2xl shadow-md px-6 py-6 md:py-10 bg-white/60 
//                 ring-1 ring-indigo-200 backdrop-blur-xl"
//               initial={{ opacity: 0, x: 60 }}
//               animate={{ opacity: 1, x: 0 }}
//               transition={{ delay: 0.22, duration: 0.6 }}
//             >
//               <motion.h1
//                 className="text-2xl sm:text-4xl font-black bg-clip-text text-transparent bg-gradient-to-r from-blue-800 to-indigo-600"
//                 initial={{ scale: 0.94, opacity: 0.4 }}
//                 animate={{ scale: 1, opacity: 1 }}
//                 transition={{ duration: 0.42 }}
//                 style={{
//                   letterSpacing: "0.03em",
//                   textShadow: "0 1px 6px #60a5fa44",
//                 }}
//               >
//                 {product.title}
//               </motion.h1>
//               <motion.pre
//                 className="whitespace-pre-wrap font-sans text-md sm:text-xl text-gray-800/90 leading-relaxed"
//                 initial={{ opacity: 0, y: 16 }}
//                 animate={{ opacity: 1, y: 0 }}
...
//               >
//                 <span className="text-lg md:text-xl font-semibold text-sky-800">
//                   Price:
//                 </span>
//                 {/* Animated Price "pulse" on mount */}
//                 <motion.span
//                   className="text-2xl md:text-3xl font-extrabold py-1 px-4 rounded-xl bg-gradient-to-br from-yellow-200 via-amber-100 to-transparent shadow"
...
//                   ${
//                     user
//                       ? "bg-gradient-to-r from-blue-600 via-indigo-500 to-sky-500 hover:from-indigo-700 hover:to-blue-400 cursor-pointer"
//                       : "bg-gray-300 text-gray-600 cursor-not-allowed"
//                   }`}
//                 style={{
//                   boxShadow: "0 2px 16px #60a5fa66, 0 0px 64px #a7f3d066",
//                 }}
//                 whileTap={
//                   user ? { scale: 0.97, backgroundColor: "#2563eb" } : {}
//                 }
//                 animate={
//                   user
//                     ? {
...
//                 {user ? "Add to Cart" : "Login to Add to Cart"}
//               </motion.button>
//               {/* Animated "Limited" badge example */}
//               {product.isLimited && (
//                 <motion.div
//                   className="absolute -top-3 left-3 px-3 py-1 rounded-full text-white font-semibold bg-red-400 bg-opacity-90 shadow-lg"
...
//         </div>
//       )}
//       {/* Custom pointer effect for desktop (advanced: optional, can use third-party libs for full control) */}
//       <style>
//         {`
//           @media (pointer: fine) {
...
function ProductPage() {
  ...
  const thumbnails = product
    ? [product.imageURL, product.image1, product.image2, product.image3].filter(Boolean)
    : [];

  const AnimatedBadge = ({ children }) => (
    <motion.div
      className="absolute top-2 right-2 px-3 py-1 rounded-full font-semibold text-xs tracking-wide text-indigo-900 bg-indigo-300 bg-opacity-90 shadow-lg z-20"
      initial={{ scale: 0.7, opacity: 0, rotate: -12 }}
      animate={{ scale: 1.05, opacity: 1, rotate: [0, -6, 4, 0] }}
      transition={{
        type: "spring",
          ...
          >
              ...
              <div className="flex flex-row md:flex-row justify-center md:justify-start gap-3">
                {thumbnails.map((img, idx) => (
                  <motion.img
                    ...
                    style={{
                      filter:
                        img === mainImage ? "drop-shadow(0px 0px 8px #5a67d8)" : undefined,
                    }}
                  />
                ))}
              </div>
            ...
            >
              ...
              >
                ...
                <motion.span
                  className="text-2xl md:text-3xl font-extrabold py-1 px-4 rounded-xl bg-gradient-to-br from-indigo-200 via-indigo-100 to-transparent shadow-inner"
                  initial={{ scale: 0.92, backgroundPosition: "0% 50%" }}
                  animate={{
                    scale: [1, 1.08, 1],
                    backgroundPosition: ["0% 50%", "100% 50%", "0% 50%"],
              ...
              <motion.button
                ...
                whileTap={user ? { scale: 0.97, backgroundColor: "#4c51bf" } : {}}
                animate={
                  user
                    ? {
                        boxShadow: [
